...
function NodeManager() {
    ...
    self.save = function (node, errors, callback) {
        ...
        var url = '/api/admin/node/' + (isUpdate ? node.id() : '');
    ...
    self.delete = function (node) {
        var deleteUrl = '/api/admin/node/' + node.id();
...
function ConnectionManager() {
    ...
    self.save = function (connection, errors) {
        ...
        var url = '/api/admin/connection/' + (isUpdate ? connection.id() : '');
        var method = isUpdate > 0 ? 'PUT' : 'POST';
    ...
    self.delete = function (connection) {
        var deleteUrl = '/api/admin/connection/' + connection.id();
...
function SearchModel() {
    ...
    self.selectedFloorId.subscribe(function (floorId) {
        ...
        if (floorId > 0) {
            $.getJSON('/api/admin/navigation?floorMapId=' + floorId, {
                returnformat: 'json'
            }).done(function (data) {
